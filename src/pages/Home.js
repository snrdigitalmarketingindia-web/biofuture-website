...
export default function Home() {
  ...
  return (
    <main>
      {/* Hero */}
      <section style={{background:'linear-gradient(135deg,#f6fbf0 0%,#e8f5d8 100%)',padding:'64px 0 56px',borderBottom:'1px solid #d4e8b0'}}>
        <div className="container" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:40,alignItems:'center'}}>
          <div>
            <div className="drdo-badge" style={{marginBottom:20}}>✓ DRDO Certified Technology</div>
            <h1 style={{fontFamily:'var(--font-display)',fontSize:42,lineHeight:1.2,color:'var(--green-dark)',marginBottom:16}}>
          ...
          <div style={{background:'#fff',border:'1px solid #d4e8b0',borderRadius:16,height:280,display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column',gap:8}}>
            <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#97C459" strokeWidth="1.5"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
            <span style={{fontSize:13,color:'#aaa'}}>Hero product image</span>
            <span style={{fontSize:11,color:'#ccc'}}>Add your best product photo here</span>
          </div>
        </div>
      </section>

      {/* Trust badges */}
      <section style={{padding:'40px 0',borderBottom:'1px solid #f0f5e8'}}>
        <div className="container">
          <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:16}}>
            {[
              ...
              <div key={b.title} style={{textAlign:'center',padding:'20px 12px',background:'var(--green-pale)',borderRadius:12,border:'1px solid #e0efc8'}}>
      ...
      <section style={{padding:'48px 0',background:'var(--green-pale)',borderBottom:'1px solid #d4e8b0'}}>
        <div className="container" style={{textAlign:'center'}}>
          ...
          <div className="pills-row" style={{justifyContent:'center',marginBottom:16}}>
            {customers.map(c => <span key={c} className="pill">{c}</span>)}
          </div>
          <p style={{fontSize:13,color:'#777',marginTop:8}}>Size & color customization available for bulk orders</p>
        </div>
      </section>

      {/* Enquiry form */}
      <section id="enquiry" style={{padding:'64px 0'}}>
        <div className="container">
