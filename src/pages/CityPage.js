...
export default function CityPage({ data }) {
  ...
  const anchorId = `${data.key}-enquiry`;
  ...
  const localBusiness = {
    ...
    description: `DRDO certified biodegradable carry bag manufacturer supplying ${data.name} and pan India.`,
    serviceArea: { '@type': 'City', name: data.name },
  ...
  return (
    <main>
        ...
        <div className="container">
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 14 }}>
            {data.heroTags.map(tag => (
              <span key={tag} className="drdo-badge" style={tag.includes('🚛') ? { background: 'rgba(255,255,255,0.18)', borderColor: 'rgba(255,255,255,0.4)', color: '#fff' } : tag.includes('🌿') ? { background: '#5cb81c', borderColor: '#5cb81c', color: '#fff' } : {}}>
                {tag}
              </span>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 24 }}>
            <a href={`#${anchorId}`} className="btn-primary">{data.ctaText}</a>
      ...
      <section style={{ padding: '56px 0', borderBottom: '1px solid #eef5e4' }}>
        <div className="container">
          <h2 className="section-title" style={{ textAlign: 'center' }}>Why {data.name} businesses choose us</h2>
      ...
      <section style={{ padding: '56px 0', background: 'var(--green-pale)', borderBottom: '1px solid #d4e8b0' }}>
        <div className="container">
          <h2 className="section-title" style={{ textAlign: 'center' }}>Industries we serve in {data.name}</h2>
          <p className="section-sub" style={{ textAlign: 'center' }}>Bulk biodegradable packaging for every sector</p>
      ...
      <section style={{ padding: '56px 0', borderBottom: '1px solid #eef5e4' }}>
        <div className="container">
          ...
          <p className="section-sub" style={{ textAlign: 'center' }}>All products shipped to {data.name} from our Hyderabad manufacturing facility</p>
          <div style={{ marginTop: 32, overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
              <thead>
                <tr style={{ background: 'var(--green-mid)', color: '#fff' }}>
                  {['Type', 'Sizes Available', 'Microns', 'Load Capacity', 'Best For'].map(h => (
          ...
          <p style={{ fontSize: 13, color: '#888', marginTop: 14, textAlign: 'center' }}>Custom sizes available on request.</p>
        </div>
      </section>
      ...
      <section style={{ padding: '56px 0', borderBottom: '1px solid #eef5e4' }}>
        <div className="container" style={{ maxWidth: 760 }}>
          <h2 className="section-title" style={{ textAlign: 'center' }}>Frequently asked questions — {data.name}</h2>
      ...
      {/* Enquiry */}
      <section id={anchorId} style={{ padding: '56px 0', background: 'var(--green-pale)' }}>
        <div className="container">
          <h2 className="section-title" style={{ textAlign: 'center' }}>Request {data.name} Bulk Quote</h2>
